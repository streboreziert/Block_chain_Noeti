...
function paintAccountUI() {
  ...
  if (btnSignup) btnSignup.hidden = logged;
  if (btnLogout) btnLogout.hidden = !logged;
  if (accountBtn) {
    accountBtn.textContent = logged ? shortAddress(username) : "Account";
    accountBtn.title = logged ? `${username} · ${shortAddress(w?.address)}` : "Login / Sign up";
  }
  paintGuestBanner();
}
...
function paintGuestBanner() {
  ...
  if (!banner) return;
  const show = !isLoggedIn() && !guestBannerDismissed;
  banner.hidden = !show;
}
...
function openAuthModal(mode = "login") {
  ...
  const migrate = document.getElementById("authMigrateHint");
  const err = document.getElementById("authError");
  const submitLabel = document.getElementById("authSubmitLabel");
  const submitHint = document.getElementById("authSubmitHint");
  ...
  if (tabLogin) tabLogin.classList.toggle("active", authModalMode === "login");
  if (tabSignup) tabSignup.classList.toggle("active", authModalMode === "signup");
  if (confirmWrap) confirmWrap.hidden = authModalMode !== "signup";
  if (migrate) migrate.hidden = !(authModalMode === "signup" && hasGuestWalletWithoutVault());
  if (title) title.textContent = authModalMode === "signup" ? "Sign up" : "Login";
  if (body) {
    body.textContent =
      authModalMode === "signup"
        ? "Create a username + password. Your wallet private key is encrypted on this device."
        : "Unlock your saved wallet with username + password.";
  }
  if (submitLabel) submitLabel.textContent = authModalMode === "signup" ? "Sign up" : "Login";
  if (submitHint) {
    submitHint.textContent =
      authModalMode === "signup" ? "Encrypt & save wallet" : "Unlock your wallet";
  }
  const pw = document.getElementById("authPassword");
  if (pw) pw.autocomplete = authModalMode === "signup" ? "new-password" : "current-password";
  modal.hidden = false;
}
...
async function refreshWalletUI() {
  ...
  paintAccountUI();
  if (!browserWallet) {
    if (addr) addr.textContent = "no wallet — Sign up or: wallet create";
...
async function runCommand(raw) {
  ...
  try {
    ...
    if (cmd === "login") {
      openAuthModal("login");
      term("login — enter username + password in the dialog", "info");
      return;
    }
    if (cmd === "signup") {
      openAuthModal("signup");
      term("signup — create username + password in the dialog", "info");
      return;
    }
    if (cmd === "logout") {
      authLogout();
      browserWallet = null;
      await refreshWalletUI();
      term("logged out — vault kept on device", "ok");
      toast("logged out");
      return;
    }
    if (cmd === "ask") {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("accountBtn")?.addEventListener("click", () => {
    if (isLoggedIn()) {
      const s = getSession();
      toast(`${s.username} · ${shortAddress(s.wallet?.address)}`);
      term(`whoami user=${s.username} wallet=${shortAddress(s.wallet?.address)}`, "ok");
      return;
    }
    openAuthModal("login");
  });
  ...
  document.getElementById("guestBannerSignup")?.addEventListener("click", () => openAuthModal("signup"));
  document.getElementById("guestBannerDismiss")?.addEventListener("click", () => {
  ...
  document.getElementById("authForm")?.addEventListener("submit", async (e) => {
    ...
    try {
      if (authModalMode === "signup") {
        const r = await authSignup({ username, password, confirm, migrateGuest: true });
        browserWallet = r.wallet;
        term(`signed up as ${r.username} · ${shortAddress(r.wallet.address)}`, "ok");
        toast(`welcome, ${r.username}`);
      } else {
        const r = await authLogin({ username, password });
        browserWallet = r.wallet;
        term(`logged in as ${r.username} · ${shortAddress(r.wallet.address)}`, "ok");
        toast(`hi, ${r.username}`);
      }
      closeAuthModal();
      await refreshWalletUI();
